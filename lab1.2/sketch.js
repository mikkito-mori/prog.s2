...
function setup() {
    ...
    // Создаем контейнер для элементов управления звуком
    ...
    // Создаем слайдер громкости с кастомным стилем
    ...
    // Кастомный стиль для слайдера (ползунок)
...
function drawScore() {
    // Создаем стилизованный контейнер для счета
    fill(255, 255, 255, 180); // Белый с прозрачностью
    stroke(0, 100, 0); // Темно-зеленая обводка
    strokeWeight(2);
    rect(10, 10, 150, 40, 20); // Скругленные углы
    
    // Текст "Счет"
    fill(0, 100, 0); // Темно-зеленый цвет
    noStroke();
    textSize(18);
    ...
    // Окно с очками
    fill(255, 255, 200); // Светло-желтый фон
    stroke(0, 100, 0);
    rect(80, 15, 70, 30, 10); // Скругленные углы
    ...
    // Маленькая иконка семечки справа
    fill(210, 180, 140);
    noStroke();
